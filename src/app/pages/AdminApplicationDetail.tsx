          ...
          <section key={section.title} className="border border-black/10 bg-white">
            ...
            {repeatables.map((rep) => {
              ...
              return (
                <div
                  key={rep.path}
                  className="overflow-x-auto border-t border-black/10"
                >
                      ...
                      {rows.map((row, i) => (
                        ...
                        >
                          {rep.columns.map((col) => (
                            <td
                              key={col.path}
                              className="px-4 py-3 text-mico-dark"
                            >
...
export default function AdminApplicationDetail() {
  ...
  const dataField = (application as Application & { data?: AdmissionApplicationData })
    .data;
  const hasWizardData = Boolean(
    dataField &&
      typeof dataField === "object" &&
      "firstChoiceProgramme" in dataField,
  );
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          ...
          {!hasWizardData ? (
            <button
              type="button"
              onClick={() =>
                void downloadApplicationPdf(
                  toValues(application),
        ...
        {hasWizardData ? (
          <WizardDetail
            application={
              {
                ...application,
                data: dataField as AdmissionApplicationData,
              }
            }
          />
        ) : (
